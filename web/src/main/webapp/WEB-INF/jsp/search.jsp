...
<div class="navbar-collapse collapse" id="searchBar">
    ...
    <h1>Результаты поиска:<br>
        <small>Выберите понравившиеся вам обои и добавьте их в корзину.</small>
    </h1>
    <c:if test="${user==null}">
    ...
    <c:forEach var="goods" items="${result}">
        <div class="ad">
            ...
            <div class="price">
                ...
                <c:if test="${goods.getSale() == 0}">
                    Цена: $<c:out value="${goods.getPrice()}"/>
                </c:if>
            </div>
            <c:if test="${user!=null}">
...
<ul class="pagination">
    ...
    <c:forEach var="count" begin="1" end="${pagesCount}">
        <c:if test="${page == count}">
            <li class="active"><a href="<c:url value="/search"/>?&page=${count}&search=${search}">${count}</a></li>
        </c:if>
        <c:if test="${page != count}">
            <li><a href="<c:url value="/search"/>?page=${count}&search=${search}">${count}</a></li>
        </c:if>
    </c:forEach>
</ul>